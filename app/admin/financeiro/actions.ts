...
export async function createPayment(_prev: CreatePaymentResult | null, formData: FormData): Promise<CreatePaymentResult> {
  ...
  const { error } = await supabase.from("Payment").insert({ id, studentId, amount, status, referenceMonth });
